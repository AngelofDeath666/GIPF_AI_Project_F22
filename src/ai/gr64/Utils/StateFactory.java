...
public class StateFactory {
    ...
    public static GameState create(int layers, StartingPieces startingMode, int startingPieces) {
        ...
        int[] layerEnds = BoardUtils.LayerEnds(layers);

        GenerateNodes(nodes, layerEnds);

        AddNeighbors(nodes, layerEnds);

        SetupStartingPieces(startingMode, nodes, layers);
    ...
    private static void GenerateNodes(INode[] nodes, int[] layerEnds) {
    ...
    private static void AddNeighbors(INode[] nodes, int[] layerEnds){
    ...
    private static void SetupStartingPieces(StartingPieces startingMode, INode[] nodes, int layers) {
        ...
        */
        switch (startingMode) {
            ...
            case NORMAL:
                for (int i = 0; i < 6; i++) {
                    INode workingNode = nodes[nodes.length / 2];
